function [h,asp,grad] = hillshade(dem,X,Y,varargin)
...
[fx,fy] = gradient(dem,dx,dy); % uses simple, unweighted gradient of immediate neighbours,fx-gradient along longitude, fy-gradient along latitude
[asp,grad]=cart2pol(fy,fx);    % convert to carthesian coordinates, asp and grad in unit of in radians,asp is a counterclockwise angular displacement in radians from the positive x-axis, 
%!!!!!! cart2pol计算asp，从北向南为positive x-axis，　所以east对应π/2，west对应-π/2
%grad = grad/d; % multiply w cellsize
grad=atan(zf*grad); % steepest slope
asp=-asp; % 转换成计算costheta需要的asp定义，也即asp=0 for due south, asp = -π/2 for east, asp  = π/2 for west and γ = ± π for north. 
% convert asp
